...
void main() {
  ...
  testWidgets('navigate to', (tester) async {
    ...
    expect(NavigationBuilder.context, isNotNull);
    expect(Navigator.of(NavigationBuilder.context!), isNotNull);

    expect(find.text('Home'), findsOneWidget);
    _navigator.toPageless(
      const Route1('data'),
    ...
    // _navigator.toReplacement('Route2', result: '');
    // await tester.pumpAndSettle();
  ...
  //   await tester.pumpAndSettle();
  //   //
  //  _navigator.toAndRemoveUntilPageless(
  //     Route2('data'),
  //     name: 'ROUTE2',
  //     untilRouteName: '/',
  //     fullscreenDialog: true,
  ...
  //   await tester.pumpAndSettle();
  //   //
  //   _navigator.navigate.toAndRemoveUntil(
  //     Route1(''),
  ...
  testWidgets('navigate to named remove  all', (tester) async {
    ...
    await tester.pumpAndSettle();
    //
    _navigator.toAndRemoveUntil('Route3', arguments: 'data');
    await tester.pumpAndSettle();
    expect(find.text('Route3'), findsOneWidget);
    //
    _navigator.back();
  ...
  testWidgets('WHEN _navigator.navigate.pageRouteBuilder is defined'
    ...
    NavigationBuilder.pageRouteBuilder = (Widget nextPage, settings) =>
        PageRouteBuilder(
          settings: settings,
          transitionDuration: const Duration(milliseconds: 2000),
          reverseTransitionDuration: const Duration(milliseconds: 2000),
          pageBuilder: (context, animation, secondaryAnimation) => nextPage,
          transitionsBuilder: NavigationBuilder.transitions.bottomToUp(),
        );
  ...
  testWidgets('WHEN _navigator.navigate.transitionsBuilder is defined'
      'Route animation uses it'
      'CASE Widget route', (tester) async {
    // _navigator.disposeAll();
    NavigationBuilder.transitionsBuilder = NavigationBuilder.transitions
        .leftToRight(duration: const Duration(milliseconds: 2000));
    ...
    await tester.pump(const Duration(seconds: 1));
    expect(find.text('Route1: data'), findsOneWidget);
    //TODO check me (is it a good idea to add toReplacementPageless)
    // _navigator.navigate.toReplacement(const Route2('data'), result: '');
    _navigator.toReplacement('/Route2', result: '', arguments: 'data');
    await tester.pump(const Duration(seconds: 1));
    expect(find.text('Route1: data'), findsOneWidget);
    expect(find.text('Route2: data'), findsNothing);
    await tester.pump(const Duration(seconds: 1));
  ...
  testWidgets(
    'WHEN transitionsBuilder is defined in _navigator.navigate.onGenerateRoute'
    'THEN it will work',
    (tester) async {
  ...
  //     navigatorKey: _navigator.navigate.navigatorKey,
  //     onGenerateRoute: _navigator.navigate.onGenerateRoute(
  //       {
  //         '/': (c) => Text('Home'),
  //         'Route1': (param) => Route1(param as String),
  //         'Route2': (param) => Route2(param as String),
  //         'Route3': (c) => Text('Route3'),
  //       },
  //     ),
  //   );
  ...
  testWidgets('WHEN undefined name route is given'
  ...
  //         //   }
  //         // },
  //         unknownRoute: (c) => Text('Unknown Route'),
  //       ),
  //     );
  //     await tester.pumpWidget(app);
  ...
  testWidgets('WHEN  mixing query params and path params'
      'THEN both are parsed', (tester) async {
    final app = _TopWidget(
      routers: {
        '/': (c) => const Text('Home'),
        '/Route1/:id': (param) {
    ...
    _navigator.to('Route1/toNamed,', queryParams: {'data': ' Parsed data : 2'});
    await tester.pumpAndSettle();
    ...
    await tester.pumpAndSettle();
    expect(
      find.text('Route1: toReplacementNamed, Parsed data : 2'),
      findsOneWidget,
    );
    ...
    await tester.pumpAndSettle();
    expect(
      find.text('Route1: toNamedAndRemoveUntil, Parsed data : 2'),
      findsOneWidget,
    );
  });

  testWidgets('Nested route trial', (tester) async {
    final home = (c) => const Text('/');
    final page00 = (c) => Text('$c');
    final page1 = (c) => Text('$c');
    final page11 = (c) => Text('$c');
    final page111 = (c) => Text('$c');
    final page1121 = (c) => Text('$c');
    final page1122 = (c) => Text('$c');
    final page12 = (c) => Text('$c');
    final page13 = (c) => Text('$c');
    final page2 = (c) => Text('$c');
    final page3 = (c) => Text('$c');
    final page4 = (c) => Text('$c');
    final page5 = (c) => Text('$c');
    final page52 = (c) => Text('$c');
    final page521 = (c) => Text('$c');
    ...
    final routes = {
      '/': home,
      '/page0': (c) =>
          RouteWidget(routes: {'/page00': (c) => page00(c.arguments)}),
      '/page1': (c) => RouteWidget(
        routes: {
          '/': (c) => page1(c.arguments),
          '/page11': (c) => RouteWidget(
            routes: {
              '/': (c) => page11(c.arguments),
              '/page111': (c) => page111(c.arguments),
              '/page112': (c) => RouteWidget(
                routes: {
                  '/page1121': (c) => page1121(c.arguments),
                  '/page1122': (c) => page1122(c.arguments),
                },
              ),
            },
          ),
          '/page12': (c) =>
              RouteWidget(routes: {'/': (c) => page12(c.arguments)}),
          '/page13': (c) => page13(c.arguments),
        },
      ),
      '/page2': (c) => page2(c.arguments),
      '/page3': (c) => RouteWidget(builder: (__) => page3(c.arguments)),
      '/page4': (c) => page4(c.arguments),
            ...
            return RouteWidget(
              routes: {
                ...
                '/:page521ID': (c) => Builder(
                  builder: (ctx) {
                    queryParams = ctx.routeData.queryParams;
                    assert(c.queryParams == queryParams);
                    pathParams = ctx.routeData.pathParams;
                    routePath = ctx.routeData.path;
                    baseUrl = ctx.routeData.baseLocation;
  ...
  testWidgets('WHEN Nested routes are defined using RouteWidget'
      'THEN context.routeWidget get the right subRoute', (tester) async {
    final home = (c) => const Text('/');
    final page1 = (c) => Text('$c');
    final page11 = (c) => Text('$c');
    final page111 = (c) => Text('$c');
    final page112 = (c) => Text('$c');
    final page1121 = (c) => Text('$c');
    final page1122 = (c) => Text('$c');
    final page12 = (c) => Text('$c');
    // final page2 =  (c) => Text('$c');
  ...
  testWidgets('WHEN RouteWidget is defined with builder and without routes'
      'AND WHEN the exposed route or the route from the context is used'
      'THEN it throws non defined sub routes assertion', (tester) async {
    final routes = {
      '/': (data) => RouteWidget(
        builder: (route) {
          return const Text('');
  ...
  testWidgets('WHEN RouteWidget builder do not use the route'
      'THEN it works normally even if route is not found', (tester) async {
    final routes = {
      '/': (data) => RouteWidget(
        builder: (route) {
          return const Text('builder/');
        },
        routes: {'/': (data) => const Text('/')},
      ),
      '/page1': (data) => RouteWidget(
        builder: (route) {
          return Builder(
            builder: (context) {
              return const Text('builder/page1');
            },
          );
        },
        routes: {
          '/': (data) => const Text('/page1'),
          '/page11': (data) => const Text('/page11'),
        },
      ),
    };
  ...
  testWidgets('context.routeBaseUrl get the right value', (tester) async {
    RouteData? data1;
    RouteData? data2;
    RouteData? data3;
                    ...
                    routes: {
                      '/': (data) => Builder(
                        builder: (context) {
                          ...
                          return Text('page21/' + data.pathParams['user']!);
                        },
                      ),
                    },
                  ),
  ...
  testWidgets('Test route is not found', (tester) async {
    final Map<String, Widget Function(RouteData)> routes = {
      '/page1': (c) => const Text(''),
      '/page2': (c) => RouteWidget(builder: (c) => const Text('')),
      '/page3': (c) => RouteWidget(
        routes: {
          '/': (c) => const Text(''),
          '/page31': (c) {
            return RouteWidget(builder: (c) => const Text(''));
          },
        },
      ),
      '/page4': (c) => RouteWidget(
  ...
  testWidgets('Check we get the right baseUrl', (tester) async {
    RouteData? routeData;
    final routes = {
      '/': (data) => const Text('/'),
      '/page1': (data) => RouteWidget(
          ...
          '/page11': (d) {
            return Builder(
              builder: (context) {
                ...
                final ctxData = context.routeData;
                assert(routeData == ctxData);
                return const Text('/page1/page11');
              },
            );
          },
        },
      ),
      '/page2/:id': (c) => RouteWidget(
        builder: (r) {
          return r;
        },
        routes: {
          ...
          '/page21': (d) {
            return Builder(
              builder: (context) {
                ...
                final ctxData = context.routeData;
                assert(routeData == ctxData);
                return Text('/page2/1/page21: id=' + ctxData.pathParams['id']!);
              },
            );
          },
        },
  ...
  testWidgets('WHEN the same route is pushed twice'
      'THEN the new route is ignored', (tester) async {
    final routes = {
      '/': (c) {
        return Builder(
          builder: (context) {
  ...
  testWidgets(
    ...
    (tester) async {
      final routes = {
        ...
        '/page1': (c) => const Text('/page1'),
        '/page2': (c) => const Text('/page2'),
        '/page3': (c) => const Text('/page2'),
      };
      final widget = _TopWidget(routers: routes);
      await tester.pumpWidget(widget);
      dynamic message;
  ...
  testWidgets('Check backUntil'
      'Case without sub routes', (tester) async {
    final routes = {
      '/': (c) => const Text('/'),
      '/page1': (c) => const Text('/page1'),
      '/page2': (c) => const Text('/page2'),
      '/page3': (c) => const Text('/page3'),
    };
    final widget = _TopWidget(routers: routes);
    await tester.pumpWidget(widget);
    _navigator.to('/page1');
  ...
  testWidgets('Check backUntil'
      'Case with sub routes', (tester) async {
    final routes = {
      '/': (c) => const Text('/'),
      '/page1': (c) => RouteWidget(
        builder: (c) => c,
  ...
  testWidgets('Check toReplacement'
      'Case without sub routes', (tester) async {
    final routes = {
      '/': (c) => const Text('/'),
      '/page1': (c) => const Text('/page1'),
  ...
  testWidgets('Check toNamedAndRemoveUntil'
      'Case without sub routes', (tester) async {
    final navigator = NavigationBuilder.create(
      transitionDuration: const Duration(seconds: 1),
      routes: {
        '/': (c) => Scaffold(appBar: AppBar(title: const Text('/'))),
        '/page1': (c) => Scaffold(appBar: AppBar(title: const Text('/page1'))),
        '/page2': (c) => Scaffold(appBar: AppBar(title: const Text('/page2'))),
        '/page3': (c) => Scaffold(appBar: AppBar(title: const Text('/page3'))),
      },
    );
    final widget = MaterialApp.router(routerConfig: navigator.routerConfig);
    await tester.pumpWidget(widget);
    expect(find.text('/'), findsOneWidget);
  ...
  testWidgets('Check toNamedAndRemoveUntil functionality'
      'Case with sub routes', (tester) async {
    final navigator = NavigationBuilder.create(
      transitionDuration: const Duration(seconds: 1),
      routes: {
        ...
        '/page1': (c) => RouteWidget(
          routes: {
            '/': (data) =>
                Scaffold(appBar: AppBar(title: const Text('/page1'))),
            '/page11': (data) =>
                Scaffold(appBar: AppBar(title: const Text('/page11'))),
          },
        ),
        '/page2': (c) => RouteWidget(
          routes: {
            '/': (data) =>
                Scaffold(appBar: AppBar(title: const Text('/page2'))),
            '/page11': (data) =>
                Scaffold(appBar: AppBar(title: const Text('/page21'))),
          },
        ),
        '/page3': (c) => RouteWidget(
          routes: {
            '/': (data) =>
                Scaffold(appBar: AppBar(title: const Text('/page3'))),
            '/page11': (data) =>
                Scaffold(appBar: AppBar(title: const Text('/page31'))),
          },
        ),
      },
    );
  ...
  testWidgets('Check toNamedAndRemoveUntil with no untilRoute'
      'Case without sub routes', (tester) async {
    final routes = {
      '/': (c) => const Text('/'),
      '/page1': (c) => const Text('/page1'),
  ...
  testWidgets('Check toReplacement functionality'
      'Case without sub routes', (tester) async {
    final navigator = NavigationBuilder.create(
      transitionDuration: const Duration(seconds: 1),
      routes: {
  ...
  testWidgets('WHEN a dialog is pushed and back is invoked'
    ...
    await tester.pumpAndSettle();
    expect(find.text('/page2'), findsOneWidget);
    _navigator.toDialog(const AboutDialog());
    await tester.pumpAndSettle();
  ...
  testWidgets('WHEN a drawer is opened and back is invoked'
    ...
    final routes = {
      '/': (c) => wrapWithScaffold(const Text('/')),
      '/page1': (c) => RouteWidget(
        builder: (c) => c,
        routes: {'/': (c) => wrapWithScaffold(const Text('/page1'))},
      ),
      '/page2': (c) => RouteWidget(
        builder: (c) => c,
        routes: {
          '/': (c) => wrapWithScaffold(const Text('/page2')),
          '/page21': (c) => wrapWithScaffold(const Text('/page21')),
        },
      ),
    };
    final widget = _TopWidget(routers: routes);
  ...
  testWidgets('test setRouteStack', (tester) async {
    final widget = _TopWidget(
      routers: {
        '/': (c) => const Text('/'),
        '/page1': (c) => Text('/page1${c.arguments}${c.queryParams['id']}'),
      },
    );
    await tester.pumpWidget(widget);
    expect(find.text('/'), findsOneWidget);
    _navigator.setRouteStack(
      (pages) => [
        ...
        const PageSettings(name: 'new-page', child: Text('New page')),
      ],
    );
    await tester.pumpAndSettle();
    ...
    _navigator.setRouteStack(
      (pages) =>
          pages.to('/page1', arguments: 'arg', queryParams: const {'id': '1'}),
    );
    await tester.pumpAndSettle();
    expect(find.text('/page1arg1'), findsOneWidget);
    _navigator.back();
    ...
    _navigator.setRouteStack((pages) {
      ...
      return p..add(const PageSettings(name: '/name', child: Text('/name')));
    });
    await tester.pumpAndSettle();
    expect(find.text('/name'), findsOneWidget);
    _navigator.setRouteStack((pages) => pages.to('/page1', isStrictMode: true));
    await tester.pumpAndSettle();
  ...
  testWidgets('Global base url is updated when back is called'
      'THEN', (tester) async {
    final routes = {
      '/': (c) => const Text('/'),
      '/page1': (c) => RouteWidget(
  ...
  testWidgets('RouteInformationParser work as expected', (tester) async {
    ...
    final routes = {
      ...
      '/page1': (c) {
        return RouteWidget(
          ...
          routes: {
            ...
            '/page11': (c) {
              return Text('/page11-${c.queryParams['q']}');
            },
          },
        );
      },
      '/page2': (c) {
        return RouteWidget(
          ...
          routes: {
            '/': (c) {
              return Text('/page2-${c.queryParams['q']}');
            },
          },
        );
      },
    ...
    _provider!.value = RouteInformation(uri: Uri.parse('/page1/page11?q=1'));
    await tester.pumpAndSettle();
    expect(find.text('/page11-1'), findsOneWidget);
    expect(informationParser!.info!.uri.path, '/page1/page11');
    expect(informationParser!.info!.uri.queryParameters, {'q': '1'});
    expect(
      NavigationBuilderLogger.message.endsWith(
        'DeepLink to: /page1/page11?q=1',
      ),
      true,
    );
    ...
    expect(informationParser!.info!.uri.queryParameters, {'q': '1'});
    expect(
      NavigationBuilderLogger.message.endsWith('Back to: /page1/page11?q=1'),
      true,
    );
    ...
    await tester.pumpAndSettle();
    _provider!.value = RouteInformation(uri: Uri.parse('/page1/page11?q=2'));
    await tester.pumpAndSettle();
    expect(find.text('/page11-2'), findsOneWidget);
    expect(informationParser!.info!.uri.path, '/page1/page11');
    expect(informationParser!.info!.uri.queryParameters, {'q': '2'});
    ...
    _provider!.value = RouteInformation(uri: Uri.parse('/page2?q=15'));
    await tester.pumpAndSettle();
  ...
  testWidgets('test pageBuilder', (tester) async {
    final routes = {
      '/': (c) => const Text('/'),
      '/login': (c) => const Text('/login'),
    };
    ...
    final widget = _TopWidget(
      ...
      pageBuilder: (arg) {
        ...
        return _MyPage(name: arg.name, key: arg.key);
      },
    );
    await tester.pumpWidget(widget);
    expect(find.text('/'), findsOneWidget);
  ...
  testWidgets('test onBack', (tester) async {
    final routes = {
      '/': (c) => const Text('/'),
      '/form': (c) => Scaffold(appBar: AppBar(), body: const Text('/form')),
      '/page1': (c) => const Text('/page1'),
    };
    ...
    final widget = _TopWidget(
      ...
      onBack: (data) {
        ...
        if (data.location == '/form' && isFormDirty) {
          if (showDialog) {
            _navigator.toDialog(
              const AlertDialog(content: Text('')),
        ...
        if (showOtherDialog) {
          _navigator.toBottomSheet(
            const Text('toBottomSheet'),
            postponeToNextFrame: true,
          );

          _navigator.toCupertinoDialog(
            const Text('toCupertinoDialog'),
            postponeToNextFrame: true,
          );
          _navigator.toCupertinoModalPopup(
            const Text('toCupertinoModalPopup'),
  ...
  testWidgets('Text back button', (tester) async {
    RouteData? backData;
    bool exitApp = false;
    final routes = {
      '/': (c) => const Text('/'),
  ...
  testWidgets('Text shouldUseCupertinoPage', (tester) async {
    bool shouldUseCupertinoPage = false;
    final routes = {
      '/': (c) => Builder(
        builder: (context) {
    ...
    final widget = _TopWidget(routers: routes, shouldUseCupertinoPage: true);
    await tester.pumpWidget(widget);
    expect(find.text('/'), findsOneWidget);
  ...
  testWidgets('Text shouldUseCupertinoPage with CupertinoApp.router', (
    tester,
  ) async {
    bool shouldUseCupertinoPage = false;
    final routes = {
    ...
    final navigator = NavigationBuilder.create(routes: routes);
  ...
  group('redirect', () {
    testWidgets('test cyclic redirect infinite loop', (tester) async {
      final Map<String, Widget Function(RouteData)> routes = {
        '/': (c) => const Text('/'),
        '/page1': (c) => const Text('/page1'),
        '/page2': (c) => const Text('/page2'),
        '/page3': (c) => const Text('/page3'),
        '/page4': (c) => c.redirectTo('/page5'),
        '/page5': (c) => const Text('/page3'),
        '/page6': (c) => const Text('/page6'),
        '/page7': (c) => RouteWidget(
          routes: {
            '/': (c) => c.redirectTo('/page7/0'),
            '/:id': (c) => Text('/page7/${c.pathParams['id']}'),
          },
        ),
        '/page8': (c) => c.redirectTo('/page8/0'),
        '/page8/:id': (c) => Text('/page8/${c.pathParams['id']}'),
      };
      final widget = _TopWidget(
        routers: routes,
        // // debugPrintWhenRouted: true,
      ...
      await tester.pumpAndSettle();
      expect(
        find.text('404 Infinite redirect loop: (/page4, /page5)'),
        findsOneWidget,
      );
    ...
    testWidgets('Test global redirect with navigation on data of an other state'
        'THEN', (tester) async {
    ...
    testWidgets('Text global redirection', (tester) async {
      final routes = {
        '/': (c) => const Text('Not reached'),
        '/home': (c) => const Text('/home'),
        '/home1': (c) => const Text('/home1'),
        '/home2': (c) => const Text('/home2'),
        '/home3': (c) => const Text('Not reached'),
        '/home4': (c) => const Text('Not reached'),
        '/home5': (c) => const Text('Not reached'),
        '/page1': (c) => RouteWidget(
          builder: (c) => Builder(
            builder: (context) {
              return c;
      ...
      await tester.pumpAndSettle();
      expect(find.text('/home'), findsOneWidget);
      expect(_navigator.routeData.redirectedFrom?.uri.toString(), '/home4?q=1');

      // //
      _navigator.to('/home5?q=1');
      await tester.pumpAndSettle();
      expect(find.text('/page1'), findsOneWidget);
      expect(_navigator.routeData.redirectedFrom?.uri.toString(), '/home5?q=1');
      ...
      await tester.pumpAndSettle();
      expect(find.text('/home'), findsOneWidget);
      expect(_navigator.routeData.redirectedFrom?.uri.toString(), '/page2?q=1');
      //
      _navigator.toAndRemoveUntil('/');
      await tester.pumpAndSettle();
      _navigator.back();
    ...
    testWidgets('The default home route is redirected to another route', (
      tester,
    ) async {
      final widget = _TopWidget(
        routers: {
          '/': (c) {
            return c.redirectTo('/page1');
          },
          '/page1': (c) => RouteWidget(
            builder: (c) => c,
            routes: {
              '/': (c) {
                return c.redirectTo(c.location + '/page11');
              },
              '/page11': (c) {
                return RouteWidget(
                  routes: {
                    '/': (c) {
                      return c.redirectTo(c.location + '/page111');
                    },
                    '/page111': (c) {
                      return const Text('/page1');
                    },
        ...
        '/': (c) {
          return RouteWidget(
            routes: {
              ...
              '/page1': (c) {
                return c.redirectTo('/page1/popular');
              },
              '/page1/:kind(all|popular)': (c) {
                return Text(c.pathParams['kind']!);
              },
            },
          );
        },
    ...
    testWidgets('WHEN redirect is used inside a "/" routeWidget', (
      tester,
    ) async {
      ...
      final Map<String, Widget Function(RouteData)> routes = {
        ...
        '/': (c) {
          return RouteWidget(
            ...
            routes: {
              '/': (c) {
                return c.redirectTo('/books');
              },
              '/books': (c) {
                return RouteWidget(
                  routes: {
                    '/': (c) {
                      return c.redirectTo('/books/popular');
                    },
                    '/:kind(all|popular)': (c) => Text(c.pathParams['kind']!),
                  },
                );
              },
            },
    ...
    testWidgets('WHEN redirect is used inside a "/" routeWidget with builder', (
      tester,
    ) async {
      bool isSignedIn = false;
      final Map<String, Widget Function(RouteData)> routes = {
    ...
    testWidgets('WHEN redirect form RouteWidget to an outside route', (
      tester,
    ) async {
      final Map<String, Widget Function(RouteData)> routes = {
        '/': (data) => const Text('/'),
        ...
        '/books': (data) {
          return RouteWidget(
            builder: (c) {
              return SizedBox(child: c);
            },
            routes: {'/': (data) => const Text('/books')},
          );
        },
      };
      final widget = _TopWidget(
    ...
    testWidgets('WHEN redirect form RouteWidget to an inside route', (
      tester,
    ) async {
      final Map<String, Widget Function(RouteData)> routes = {
        '/': (data) => const Text('/'),
          ...
          '/': (c) {
            return RouteWidget(
              ...
              routes: {
                ...
                '/books': (data) {
                  // return const Text('/books');
                  // return c.redirectTo('/books/1');
                  return RouteWidget(
                    builder: (c) {
                      return Container(child: c);
                    },
                    routes: {
                      '/': (data) => c.redirectTo('/books/1'),
                      '/:id': (c) => Text('/books/${c.pathParams['id']}'),
                    },
                  );
                },
                // '/books/:id': (data) {
    ...
    testWidgets('Redirect to unknown route', (tester) async {
      final Map<String, Widget Function(RouteData)> routes = {
        '/': (data) => const Text('Home'),
        '/page1': (data) => RouteWidget(
          builder: (c) {
            return Center(child: c);
          },
  ...
  testWidgets('WHEN RouteWidget with "/" only are nested '
      'THEN it works ', (tester) async {
    final Map<String, Widget Function(RouteData)> routes = {
      '/': (c) {
        return RouteWidget(
          routes: {
            '/': (c) => RouteWidget(
              routes: {
                ...
                '/page1': (c) => const Text('/page1-bis'),
                '/page2': (c) => RouteWidget(
                  routes: {
                    '/': (data) => const Text('/page2'),
                    '/page22': (data) => const Text('/page22'),
                  },
                ),
              },
            ),
  ...
  testWidgets('Test delegateImplyLeadingToParent', (tester) async {
    final routes = {
      '/': (data) => const Text('/'),
      '/page1': (data) =>
          Scaffold(appBar: AppBar(), body: const Text('/page1')),
      '/page2': (data) => RouteWidget(
        routes: {
          '/': (c) => Scaffold(appBar: AppBar(), body: const Text('/page2')),
        },
      ),
      '/page3': (data) => RouteWidget(
        routes: {
          '/': (c) => RouteWidget(
            routes: {
              '/': (c) =>
                  Scaffold(appBar: AppBar(), body: const Text('/page3')),
            },
          ),
        },
      ),
      '/page4': (data) => RouteWidget(
        routes: {
          '/': (c) => RouteWidget(
            ...
            routes: {
              '/': (c) =>
                  Scaffold(appBar: AppBar(), body: const Text('/page4')),
            },
          ),
        },
      ),
      '/page5': (data) => RouteWidget(
        routes: {
          '/': (c) => RouteWidget(
            ...
            routes: {
              '/': (c) =>
                  Scaffold(appBar: AppBar(), body: const Text('/page5')),
              '/page51': (c) =>
                  Scaffold(appBar: AppBar(), body: const Text('/page51')),
            },
          ),
        },
      ),
      '/page6': (data) => RouteWidget(
        builder: (c) {
          return Scaffold(appBar: AppBar(), body: const Text('/page6'));
        },
      ),
    };
    final widget = _TopWidget(
  ...
  testWidgets('Check NavigationBuilder.builder works', (tester) async {
    _transitionsBuilder = NavigationBuilder.transitions.rightToLeft(
      duration: const Duration(seconds: 1),
    );
    final routes = {
      '/': (data) => const Redirect('/page1'),
      '/page1': (data) => RouteWidget(
        routes: {
          '/': (data) => Scaffold(appBar: AppBar(), body: const Text('/page1')),
        },
      ),
      '/page2': (data) => RouteWidget(
        routes: {
          '/': (data) => Scaffold(appBar: AppBar(), body: const Text('/page2')),
        },
      ),
    };
          ...
          body: Column(
            children: [
              ...
              ElevatedButton(
                ...
                child: const Text('ToPage1'),
              ),
              ElevatedButton(
                onPressed: () => _navigator.to('page2'),
                child: const Text('ToPage2'),
              ),
            ],
          ),
        );
    ...
    expect(find.text('/page2'), findsOneWidget);
    expect(find.byType(ElevatedButton), findsNWidgets(2));
    expect(find.byType(BackButton), findsNWidgets(2)); // should be findOnWidget
  ...
  testWidgets('Check that back is reactive', (tester) async {
    final routes = {
      '/': (data) => const Redirect('/page1/1'),
      '/page1/:id': (data) => RouteWidget(
        delegateImplyLeadingToParent: false,
        builder: (c) => Scaffold(
          appBar: AppBar(key: const Key('AppBar/page1')),
          body: c,
        ),
        routes: {
          '/': (data) => Scaffold(
          ...
          '/page2/:id': (data) => RouteWidget(
            delegateImplyLeadingToParent: false,
            builder: (c) => Scaffold(
              appBar: AppBar(key: const Key('AppBar/page2')),
              body: c,
            ),
            routes: {
              '/': (data) => Scaffold(
      ...
      builder: (c) {
        return Scaffold(
          ...
          body: Column(
            children: [
              Expanded(
                child: Builder(builder: (context) => context.routerOutlet),
              ),
            ],
          ),
        );
    ...
    final backPage1 = find.descendant(
      of: find.byKey(const Key('AppBar/page1')),
    ...
    final backPage2 = find.descendant(
      of: find.byKey(const Key('AppBar/page2')),
      matching: find.byType(BackButton),
    );
    ...
    expect(find.text('/page1/1'), findsOneWidget);
    expect(find.text('Title: /page1/1'), findsOneWidget);
    expect(find.byKey(const Key('BackButton')), findsNothing);
    //
    _navigator.to('/page1/2');
    _navigator.to('/page1/2/page2/1');
    _navigator.to('/page1/2/page2/2');
  ...
  testWidgets('Test toDeeply', (tester) async {
    final routes = {
      '/': (data) => const Text('/'),
      '/page1': (data) => const Text('/page1'),
      '/page1/page11': (data) => const Text('/page11'),
      '/page1/page11/page111': (data) => RouteWidget(
        routes: {
          '/': (data) => const Text('/page111'),
          '/page1111': (data) => const Text('/page1111'),
        },
      ),
    };
    final widget = _TopWidget(
  ...
  testWidgets('Test setRouteStack for subRoutes', (tester) async {
    final routes = {
      '/': (data) => const Text('/'),
      '/page1': (data) => RouteWidget(
        routes: {
          ...
          '/page12': (data) => const Text('/page12'),
        },
      ),
      '/page2': (data) => RouteWidget(
        routes: {
          '/': (data) => const Text('/page2'),
          '/page21': (data) => const Text('/page21'),
          '/page22': (data) => const Text('/page22'),
        },
      ),
      '/page3': (data) => const Text('/page3'),
    };
    final widget = _TopWidget(
      routers: routes,
      // debugPrintWhenRouted: true,
  ...
  testWidgets('Test animation and secondaryAnimation', (tester) async {
    Animation<double>? animation;
    Animation<double>? secondaryAnimation;
    final routes = {
      '/': (data) => Builder(
  ...
  testWidgets('test context.routeData', (tester) async {
    String location = '';
    String location1 = '';
    String location2 = '';
    String location3 = '';
    final routes = {
      ...
      '/page1/page11/page111': (data) => const Text('/page111'),
    };
    final widget = _TopWidget(
      routers: routes,
      builder: (routerOutlet) {
  ...
  testWidgets('test NavigationBuilder.onNavigate', (tester) async {
    final routes = {
      '/': (data) => const Text('/'),
      '/page1': (data) => const Text('/page1'),
      '/page1/page11': (data) {
  ...
  testWidgets('Test unKnownRoute', (tester) async {
    final datum = ['1', '2'];
    final Map<String, Widget Function(RouteData)> routes = {
      '/': (data) => const Text('/'),
      '/page1/:id': (data) {
  ...
  testWidgets('Text Mocking NavigationBuilder', (tester) async {
    final navigator = NavigationBuilder.create(
      routes: {
        '/': (data) => const Text('/'),
        '/page1': (date) => const Text('Page1'),
      },
    );

    MaterialApp.router(routerConfig: navigator.routerConfig);
    ...
    navigator.toPageless(const Text(''));
  ...
  testWidgets('Text Mocking NavigationBuilder get thr wright route location', (
    tester,
  ) async {
    bool isLogged = false;
    final navigator = NavigationBuilder.create(
  ...
  testWidgets('Test NavigationBuilder.toReplacement for nested routes', (
    tester,
  ) async {
    final navigator = NavigationBuilder.create(
      routes: {
  ...
  testWidgets('Test NavigationBuilder.toAndRemoveUntil for nested routes', (
    tester,
  ) async {
    final navigator = NavigationBuilder.create(
      routes: {
  ...
  testWidgets('Test NavigationBuilder.navigator for nested routes', (
    tester,
  ) async {
    final navigator = NavigationBuilder.create(
  ...
  testWidgets('Fix bug two pages have the same key', (tester) async {
    final navigator = NavigationBuilder.create(
      routes: {
        '/': (data) => const Text('/'),
        '/page1': (c) => const Text('Page1'),
    ...
    expect(
      navigator.pageStack.map((e) => e.name).toString(),
  ...
  group('unknown routes', () {
    testWidgets('Check different unknown route scenarios', (tester) async {
      final navigator = NavigationBuilder.create(
        initialLocation: '/page1/page2/page22/404',
        unknownRoute: (data) => const Text('404'),
        routes: {
          '/': (data) => const Text('/'),
          ...
          '/page2': (data) => RouteWidget(
            routes: {
              ...
              '/page22': (data) {
                return RouteWidget(
                  routes: {
                    '/': (data) => const Text('Page22'),
                    '/page23': (data) => const Text('Page23'),
                  },
                );
              },
            },
    ...
    testWidgets('Check ignore unknown route scenarios', (tester) async {
      final navigator = NavigationBuilder.create(
        initialLocation: '/page1/page2/page22/404',
  ...
  testWidgets('Test OnNavigateBackScope', (tester) async {
  ...
  testWidgets('Test deep link page transition is prevented', (tester) async {
  ...
  testWidgets('Test deep link page transition is prevented for nested routed'
      'THEN', (tester) async {
    final provider = SimpleRouteInformationProvider();
    ...
    expect(find.text('/page11'), findsOneWidget);
    //
    provider.value = RouteInformation(uri: Uri.parse('/page1/page11/page111'));
    await tester.pump();
    await tester.pump();
    expect(find.text('/page11'), findsNothing);
    expect(find.text('/page111'), findsOneWidget);
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      final navigator = NavigationBuilder.create(
        routes: {
          '/': (data) => Scaffold(appBar: AppBar(title: const Text('/'))),
          '/page1': (data) => RouteWidget(
            routes: {
              '/': (data) =>
                  Scaffold(appBar: AppBar(title: const Text('/page1'))),
...
class _NavigatorObserver extends NavigatorObserver {
  ...
  void didPop(Route route, Route? previousRoute) {
    messages = [route.settings.name, previousRoute?.settings.name];
  }
...
class _MyPage extends Page {
  const _MyPage({String? name, LocalKey? key}) : super(name: name, key: key);
  @override
  Route createRoute(BuildContext context) {
    return MaterialPageRoute(
      builder: (c) => const Text('_myPage'),
      settings: this,
    );
  }
}
